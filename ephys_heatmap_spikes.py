...
opto_stim = 0.5 
xlim_relative = [-20, 50]  # 20ms before to 50ms after stimulation
...
# Shift time axis so stimulation occurs at t=0 and convert to milliseconds
time_relative = (abf1.sweepX - opto_stim) * 1000  # Convert to milliseconds


plt.figure()
plt.imshow(sweep_data, aspect='auto', cmap='grey', origin='lower',
           extent=[time_relative[0], time_relative[-1], 0, num_sweeps])
# grey cmap looks nice or brw 
